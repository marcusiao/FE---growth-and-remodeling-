...
class Bottom(SubDomain):
    def inside(self, x, on_boundary):
        # Clamp the bottom wall
        return on_boundary and near(x[1], 0.0)

class Sides(SubDomain):
    def inside(self, x, on_boundary):
        # Roller boundary on the left and right walls
        return on_boundary and (near(x[0], x_min) or near(x[0], x_max))
...
Sides().mark(boundaries, 15)  # roller_id
...
iop_target_MPa = 150 * 0.0001333  # Increased from 40 to 150 mmHg to increase tissue stress
...
roller_id = 15
bc_roller = DirichletBC(V.sub(0), Constant(0.0), boundaries, roller_id)
...
for step in range(1, load_steps + 1):
    ...
    solve(F_res == 0, u, bcs=[bc_clamp, bc_roller], J=J_res, solver_parameters=solver_params)
...
sigma_homeo = 0.005  # MPa (Homeostatic von Mises stress baseline)
...
for gstep in range(1, growth_steps + 1):
    ...
    # Project hydrostatic tissue pressure (Use 1.0/2.0 for 2D tensor trace)
    p_tissue_expr = -(1.0/2.0) * tr(sigma_exact)
    p_tissue_func = project(p_tissue_expr, V_DG0_scalar, form_compiler_parameters={"quadrature_degree": 2})
    
    # Project Stress Magnitude for Stiffening Trigger
    sigma_mag_expr = sqrt(inner(sigma_exact, sigma_exact))
    ...
    # Define Dead Cells (e.g., 80% loss of original tissue volume)
    death_threshold = 0.8
    dead_mask = J_g_array < death_threshold
    ...
    P_crit = 12.0 * 0.0001333  # Increased capillary fragility threshold
    k = 2.0 / 0.0001333
    ...
    solve(F_res_growth == 0, u, bcs=[bc_clamp, bc_roller], J=J_res_growth, solver_parameters=solver_params)
...
for step in range(1, unload_steps + 1):
    ...
    # --- Solve equilibrium ---
    solve(F_res_unload == 0, u, bcs=[bc_clamp, bc_roller], J=J_res_unload, solver_parameters=solver_params)

...
for gstep in range(1, growth_steps + 1):
    ...
    if gstep % 10 == 0:
        ...
        print(f"[Remodeling Step {gstep}] Atrophy = {avg_atrophy:.4e}, Avg mu = {avg_mu:.4f} MPa, IOP = {current_iop_mmHg:.2f} mmHg")
        max_mu = np.max(mu_array)
        std_mu = np.std(mu_array)
        print(f"[Remodeling Step {gstep}] Avg mu = {avg_mu:.4f} (Max: {max_mu:.4f}, StdDev: {std_mu:.4f}), IOP = {current_iop_mmHg:.2f}")

...
# --- PURE CMT CONSTITUENT INTENSIC PROPERTIES ---
# Weighted sum perfectly matches healthy baseline: 0.45*0.2 + 0.30*0.5 + 0.25*3.0 = 1.0 * mu_val
...
# Initial baseline loading configuration using homogenized parameters
...
# ======================================================
# PURE CMT VARIATIONAL PROBLEM ASSEMBLY
# ======================================================
...
# Mechanical blocks scaled by their pure constituent volume fractions natively
psi_shear_axon  = theta_axon  * (mu_axon_pure / 2)  * (Ic_sym - 3 - 2 * ln(J_sym))
psi_shear_astro = theta_astro * (mu_astro_pure / 2) * (Ic_sym - 3 - 2 * ln(J_sym))
psi_shear_ecm   = Constant(0.25) * (mu_ecm_pure / 2)   * (Ic_sym - 3 - 2 * ln(J_sym))
# Newly up-regulated matrix tracks deformation relative to birth config snapshot
...
strain_homeo = 1.8e-2  
k_astro = 0.0010      # Cellular hypertrophy tracking velocity
d_astro = 0.20        
k_ecm_dep = 0.0010    # Extracellular reactive fibrosis velocity
d_ecm = 0.20         
...
# ======================================================
# PURE CMT CAUCHY STRESS TENSOR EXTRACTION
# ======================================================
...
psi_axon_v  = theta_axon  * (mu_axon_pure / 2)  * (Ic_var - 3 - 2 * ln(J_var))
psi_astro_v = theta_astro * (mu_astro_pure / 2) * (Ic_var - 3 - 2 * ln(J_var))
...
for gstep in range(1, growth_steps + 1):
    ...
    # Cache historical ECM mass before advancing time integration step
    ...
    # Mechanical Over-Activation Strain Signal Integration
    Phi_arr = np.maximum(0.0, (strain_mag_array - strain_homeo) / strain_homeo)
    
    # UPGRADE: Upregulate the actual Physical Volume Fractions via ODE integration loops
    ...
    # Calculate local increment for multi-configuration routing
    delta_theta_ecm_arr = theta_ecm_arr - theta_ecm_old.vector().get_local()
    
    # Local capillary blood flow evaluation (Ischemia Loop)
    ...
    # Neural Atrophy Degradation Saturation
    ...
    # Re-sum total surviving solid composition fraction space rule
    ...
    # --- Constrained Mixture: Update Reference Configuration for Newly Woven ECM ---
    ...
    # --- Save ---
    if gstep % 10 == 0:
        # Apparent homogenized modulus calculation for progress monitoring
...
psi_shear_axon_un  = theta_axon  * (mu_axon_pure / 2)  * (Ic_un - 3 - 2 * ln(J_un))
psi_shear_astro_un = theta_astro * (mu_astro_pure / 2) * (Ic_un - 3 - 2 * ln(J_un))
...
psi_axon_U  = theta_axon  * (mu_axon_pure / 2)  * (Ic_U - 3 - 2 * ln(J_U))
psi_astro_U = theta_astro * (mu_astro_pure / 2) * (Ic_U - 3 - 2 * ln(J_U))
psi_ecm_U   = Constant(0.25) * (mu_ecm_pure / 2)   * (Ic_U - 3 - 2 * ln(J_U))
